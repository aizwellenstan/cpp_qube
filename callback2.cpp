...
/*
  ...
  This programs is an example of how to setup 3 jobs and have the second depend on the first.
...
int main(QB_INT argc, QB_CHAR** argv)
{
	...
	*/
	...
	jobobj2->label("second");

	QbCallback* callback2 = new QbCallback();
	callback2->triggers("done-job-first");
	callback2->language(QB_CALLBACK_PERL);
	 ...
	 */
	callback2->code("print \"$qb::jobid running callback\\n\";\nqb::unblock($qb::jobid);");

	/* insert the callback into the job */
	jobobj2->callbacks().push(callback2);
		
	/*

	  Next, we setup the object for the third job.

	*/

	QbJob *jobobj3 = new QbJob();

	/* we'll name the third job.... "third job". */
	jobobj3->name("third job");
	...
	/* set the job type */
	jobobj3->prototype("cmdline");

	/* setup the package data specific to the 'cmdline' job type */
		/*    first we create a package object (this part is mostly C not C++) */
		qb_package* job3 = qb_package_new();

		/*    we then create an entry into the root package object */
		qb_package* cmdline3 = qb_package_new();

		/*    we setup the entry's data, name = cmdline, value = sleep 0 */
		qb_package_set_name(cmdline3, "cmdline");
		qb_package_set_value(cmdline3, "sleep 0");

		/*    then we push the entry into the root package object */
		qb_package_list_push(job3, cmdline3);

		/*    we convert the whole structure into a single string */
		QbString pack3 = qb_package_compact(job3);
		/* then we delete the old package structure just to be clean */
		qb_package_destroy(job3);
	...
	/*	
	  ...
	  submit it as blocked
	*/	
	jobobj3->status(QB_STATUS_BLOCKED);


	QbCallback* callback3 = new QbCallback();
	callback3->triggers("done-job-first");
	callback3->language(QB_CALLBACK_PERL);
	 ...
	 */
	callback3->code("print \"$qb::jobid running callback\\n\";\nqb::unblock($qb::jobid);");

	jobobj3->callbacks().push(callback3);
